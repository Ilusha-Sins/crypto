...
export default function CandlestickChart() {
  ...
  const indicatorSeries = useMemo(() => {
      if (data.length === 0) return { sma: [], ema: [], bb: null };
      return {
  ...
  const fetchHistoricalData = async (selectedInterval: string, endTime: number | null = null, limit = 500) => {
    try {
      ...
      setData((prevData) => {
        ...
        const uniqueArr = Array.from(uniqueMap.values()).sort((a: any, b: any) => a.x - b.x) as Candle[];

        if (uniqueArr.length > 0) {
            loadedRange.current.min = uniqueArr[0].x;
            loadedRange.current.max = uniqueArr[uniqueArr.length - 1].x;
            if (!endTime) setInitialView(uniqueArr);
        }
        return uniqueArr;
      });
      ...
      setVolumeData((prevVolume) => {
        ...
        return Array.from(uniqueVolMap.values()).sort((a: any, b: any) => a.x - b.x) as Volume[];
      });
  ...
  const handlePan = (chart: Chart) => {
    ...
    if (visibleMin - loadedMin < intervalMs * 20) {
      ...
      fetchHistoricalData(interval, loadedMin - 1, 500);
    }
  };
  ...
  }, [data, interval, overlays, indicatorSeries, drawTool, customPlugins]);
    ...
    volumeInstance.current = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          zoom: {
            pan: { enabled: drawTool === 'cursor', mode: "x", onPan: ({ chart }) => syncZoom(chart) },
            zoom: { enabled: true, mode: "x", onZoom: ({ chart }) => syncZoom(chart) } as any,
          },
        },
        scales: {
          x: {
          ...
          y: {
             position: 'right',
             grid: { display: false },
             ticks: { maxTicksLimit: 3, callback: (val) => formatVolume(val as number) },
             afterFit: (axis: any) => { axis.width = 65; }
          },
        },
      },
    ...
    try {
        ...
        wsRef.current.onmessage = (event) => {
          ...
          if (message.e === "kline") {
            ...
            const volume: Volume = { x: k.t, y: parseFloat(k.v) };
  ...
  const scrollChart = (direction: -1 | 1) => {
    ...
    if (direction === -1) handlePan(chart);
  };
  ...
  return (
    <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 space-y-8 flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[60px_1fr] gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 relative flex flex-col min-h-[600px]">
          <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
             ...
             <button onClick={resetView} className="text-sm text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1 rounded-lg transition-colors">Скинути</button>
          </div>
          ...
          {errorMsg && !isLoading && (
            <div className="absolute inset-0 bg-white/90 z-10 flex flex-col items-center justify-center rounded-xl">
               <span className="text-red-500 text-lg font-semibold mb-2">Помилка: {errorMsg}</span>
               <button onClick={() => fetchHistoricalData(interval)} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Спробувати знову</button>
            </div>
          )}

          <div className="flex flex-col flex-1 relative group">
             <div className="absolute top-4 left-4 z-20 flex gap-2">
                <button onClick={() => handleManualZoom('out')} className="flex items-center justify-center w-8 h-8 bg-white/90 shadow-sm border border-gray-200 rounded-lg text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all font-bold text-lg" title="Зменшити масштаб">-</button>
                <button onClick={() => handleManualZoom('in')} className="flex items-center justify-center w-8 h-8 bg-white/90 shadow-sm border border-gray-200 rounded-lg text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all font-bold text-lg" title="Збільшити масштаб">+</button>
             </div>
            ...
            <div className={`flex-grow relative w-full h-[75%] ${drawTool !== 'cursor' ? 'cursor-crosshair' : 'cursor-default'}`}>
      ...
      {/* Middle Row: Risk Calculator + Order Book (Equal Height) */}
